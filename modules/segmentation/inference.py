...
class SegmentationModel:
    def __init__(self, weights_path, device='cuda', conf_thresh=0.25, iou_thresh=0.45):
    ...
    def process_frame(self, frame, frame_idx, timestamp_sec):
        ...
        # Optimized Rendering Overlay
        overlay = frame.copy().astype(np.float32)
        mask_data = masks.data.cpu().numpy()
        mask_segments = masks.xy 

        # Pass 1: Apply mask transparency overlays (float32 for smooth blending)
        ...
        # Pass 2: Draw contours, bboxes, and labels on uint8 frame
        for i, (box, mask) in enumerate(zip(boxes, mask_data)):
            cls_id = int(box.cls[0].item())
            cls_name = self.names.get(cls_id, f"cls_{cls_id}")

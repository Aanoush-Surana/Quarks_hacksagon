...
class SegmentationModel:
    def __init__(self, weights_path, device=None, conf_thresh=0.25, iou_thresh=0.45):
        ...
        # Keep FP32 to preserve accuracy
        if self.device != "cpu":
    ...
    def process_frame(self, frame, frame_idx, timestamp_sec):
        ...
        if masks is not None and boxes is not None:
            ...
            for i, box in enumerate(boxes):
                ...
                color_np = np.array(bgr_colour, dtype=np.float32)

                # Convert only required mask
                ...
                # Store JSON (avoid unnecessary conversions)
                poly_points = mask_segments[i] if i < len(mask_segments) else []
        ...
        # Convert once to uint8
        final_frame = overlay.astype(np.uint8)

        # Draw boxes + labels AFTER overlay (cheaper)
